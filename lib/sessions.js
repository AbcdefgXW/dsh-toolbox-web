...
export async function listAllSessions() {
  // 【性能】并发请求共用同一次扫描：前端打开工具箱时会**同时**拉「会话列表」(sessions.list)
  // 与「子目录计数」(workspace.list → refreshSessionCounts)，两者都走这里。
  // 一份 217 会话的目录遍历在本机实测数秒，重复扫描是纯浪费（表现为"子目录也要等"）。
  // 注意：只共享「进行中」的扫描，完成后立即解除——不保留结果，因此不会返回过时数据。
  if (_inflight) return _inflight;
  _inflight = scanAllSessions();
  try {
...
async function scanAllSessions() {
  ...
  for (const projectDir of fs.readdirSync(SESSIONS_ROOT)) {
    ...
    for (const sessionDir of fs.readdirSync(projectPath)) {
      ...
      // 【性能】定期让出事件循环：逐条读 header 在本机实测 217 个耗时数秒，
      // 不让出会让这一个请求长期独占主线程，导致用户在面板里切「预设 / 配置文件 / 子目录」时全部排队。
      // 粒度取 8：粒度越细，其它请求插空越快（代价仅是让出开销，可忽略）。
      if ((++n & 7) === 0) await new Promise((r) => setImmediate(r));
    }
  }
  return result;
}
